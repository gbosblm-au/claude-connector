...
// v12.28.0 (TNX-C-009): hard ceiling on media fetched for upload to WordPress.
const MAX_MEDIA_DOWNLOAD_BYTES = parseInt(process.env.MAX_MEDIA_DOWNLOAD_BYTES || "26214400", 10);
...
// rather than only the initial URL. See src/utils/safeFetch.js.
  ...
  if (!resp.ok) {
    throw new Error(`Failed to download image from ${imageUrl}: ${resp.status}`);
  }
  if (resp.truncated) {
    throw new Error(`Image at ${imageUrl} exceeds the ${MAX_MEDIA_DOWNLOAD_BYTES} byte upload limit.`);
  }
